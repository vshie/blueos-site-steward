...
const SYSTEM_WWW_DIR = process.env.SYSTEM_WWW_DIR || "/system-www";
const LABELS_FILE =
...
// Time-from-RTC sidecar status (Site Steward's own time sidecar). Legacy
// blueos-site-stack installs published under `.../ext/site-stack/json`; the
// steward publishes under `.../ext/site-steward/json` and this UI subscribes
// to whichever the operator has configured via TIME_STATUS_TOPIC.
const TIME_STATUS_TOPIC =
  process.env.TIME_STATUS_TOPIC || `${MQTT_ROOT}/ext/site-steward/json`;
...
app.get("/register_service", (req, res) => {
  res.json({
    name: "Site Steward",
    description:
      "MQTT hub, history, controls, and trends for your ESP boards.",
    icon: "mdi-home-analytics",
    company: "Community",
    version: "0.1.0",
    webpage: "https://github.com/vshie/blueos-site-steward",
    api: "https://github.com/vshie/blueos-site-steward/blob/main/README.md",
    new_page: false,
...
// Static /system/* mount — serves the former site-stack status page under the
// portal's System tab. entrypoint.sh writes /system-www/runtime.json with
// steward metadata (broker/Influx/version) that the tab reads.
app.use("/system", express.static(SYSTEM_WWW_DIR, { extensions: ["html"] }));

// Reverse-proxy /grafana/* → 127.0.0.1:3000. BlueOS mounts this app under
// /extensionv2/sitesteward/, so requests actually arrive here already stripped
// of that prefix; strip our /grafana prefix too so the request hits Grafana at
// its root. WS/SSE pass through for live-updating panels.
const grafanaProxy = createProxyMiddleware({
  target: `http://127.0.0.1:${GRAFANA_PORT}`,
  changeOrigin: true,
  ws: true,
  xfwd: true,
  pathRewrite: { "^/grafana": "" },
...
// the proxy sometimes rewrites or drops the path on upgrade. Use noServer so
// the Grafana reverse-proxy can claim its own /grafana WS upgrades below.
const wss = new WebSocketServer({ noServer: true });

server.on("upgrade", (req, socket, head) => {
  const rawUrl = req.url || "";
  const pathOnly = rawUrl.split("?", 1)[0];
  if (pathOnly === "/grafana" || pathOnly.startsWith("/grafana/")) {
    // Let http-proxy-middleware handle the Grafana WS upgrade.
    if (typeof grafanaProxy.upgrade === "function") {
      grafanaProxy.upgrade(req, socket, head);
    } else {
      socket.destroy();
    }
    return;
...
server.listen(CONTROL_PORT, "0.0.0.0", () => {
  console.log(`[http] Site Steward portal listening on :${CONTROL_PORT}`);
});
