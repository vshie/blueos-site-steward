(() => {
  ...
  function activateTab(name) {
    ...
    if (name === "graphs" && !grafanaFrameLoaded && grafanaFrameSrc) {
      grafanaFrameEl.src = grafanaFrameSrc;
      grafanaFrameLoaded = true;
    }
  ...
  fetch(api("api/health"))
    ...
    .then((h) => {
      ...
      grafanaFrameSrc = `${grafanaBase}/d/blueos-esp-sensors?orgId=1&kiosk=tv&theme=dark&refresh=30s`;
      setTimeStatus(h.timeStatus);
      updateSystemPanelFromHealth(h);
    })
    .catch(() => {});
  ...
  function paintDot(id, cls) {
    ...
    if (!el) return;
    el.className = `dot ${cls || ""}`.trim();
  }
  function setText(id, text) {
    const el = document.getElementById(id);
    if (el) el.textContent = text;
  }
  function updateSystemPanelFromHealth(h) {
    latestHealth = h || latestHealth;
    if (!latestHealth) return;
    if (latestHealth.mqttConnected === true) {
      paintDot("sys-mqtt-dot", "ok");
      setText("sys-mqtt-label", "connected");
    } else if (latestHealth.mqttConnected === false) {
      paintDot("sys-mqtt-dot", "bad");
      setText("sys-mqtt-label", "disconnected");
    } else {
      paintDot("sys-mqtt-dot", "");
      setText("sys-mqtt-label", "unknown");
    }
    const ts = latestHealth.timeStatus;
    if (ts && ts.time_source) {
      const okStates = new Set(["ntp", "esp-rtc", "esp-rtc-ok"]);
      paintDot("sys-time-dot", okStates.has(ts.time_source) ? "ok" : "bad");
      setText("sys-time-label", ts.time_source);
      paintDot(
        "sys-telegraf-dot",
        latestHealth.mqttConnected ? "ok" : ""
      );
      setText(
        "sys-telegraf-label",
        latestHealth.mqttConnected ? "running" : "unknown"
      );
    } else {
      paintDot("sys-time-dot", "");
      setText("sys-time-label", "no sample yet");
    }
  }
  function renderEndpoints() {
    const listEl = document.getElementById("sys-endpoints");
    if (!listEl) return;
    const host = location.hostname || "blueos.local";
    const proto = location.protocol || "http:";
    const grafanaPort =
      (latestHealth && latestHealth.grafanaPort) ||
      (latestRuntime && latestRuntime.grafana_port) ||
      "3000";
    const endpoints = [
      { name: "InfluxDB HTTP API", url: `${proto}//${host}:8086/ping` },
      { name: "Grafana (direct)",  url: `${proto}//${host}:${grafanaPort}` },
      { name: "MQTT (TCP)",         url: `mqtt://${host}:1883` },
      { name: "MQTT (WebSockets)",  url: `ws://${host}:9001` },
    ];
    listEl.innerHTML = "";
  ...
  function refreshSystemPanel() {
    updateSystemPanelFromHealth(latestHealth);
    renderEndpoints();
    fetch("system/runtime.json", { cache: "no-store" })
      .then((r) => (r.ok ? r.json() : null))
      .then((j) => {
        if (!j) return;
        latestRuntime = j;
        setText("sys-version", j.version || "—");
        setText("sys-db", j.database || "—");
        paintDot("sys-influx-dot", "ok");
        setText("sys-influx-label", `ready · ${j.influx_version || "1.8"}`);
        renderEndpoints();
      })
      .catch(() => {
        paintDot("sys-influx-dot", "bad");
        setText("sys-influx-label", "unavailable");
      });
    fetch(api("api/health"), { cache: "no-store" })
      .then((r) => r.json())
      .then((h) => updateSystemPanelFromHealth(h))
      .catch(() => {});
  }
  ...
  function connectWs() {
    ...
    ws.onmessage = (evt) => {
      ...
      if (msg.type === "full_state") {
        ...
        if (latestHealth) latestHealth.mqttConnected = msg.connected;
        updateSystemPanelFromHealth(latestHealth);
        ...
        if (latestHealth) latestHealth.timeStatus = msg.status;
        updateSystemPanelFromHealth(latestHealth);
      }
    };
